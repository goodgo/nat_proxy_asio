...
void CSession::reader(asio::yield_context yield)
{
	...
	while (_started) {
		...
		if (ec || 0 == bytes) {
			LOG_ERROR << "session[" << _id << "] read head error: " << ec.message();
			stop();
			break;
		}
		...
		if (ec || 0 == bytes) {
			LOG_ERROR << "session[" << _id << "] read body error: " << ec.message();;
			stop();
			break;
		}
		...
		LOG_TRACE << "session[" << _id << "] read(#REQ_" << _mgr->getFuncName(_hdr.ucFunc) << "#): "
	...
	LOG_INFO << "session[" << _id << "] reader exit!";
}

void CSession::asyncWrite(const StringPtr& pkt)
{
	if (pkt->length() <= 0)
		return;
	...
	_strand.post([self, pkt](){
		...
		if (self->_sque.size() > 1) {
			LOG_TRACE << "session[" << self->_id << "] queue size " << self->_sque.size() << " > 1 return.";
			return;
		}
		self->asyncWrite();
	});
...
void CSession::asyncWrite()
{
	...
	auto self = shared_from_this();
	asio::async_write(_socket, asio::buffer(msg->c_str(), msg->length()),
			self->_strand.wrap([self, msg](const boost::system::error_code& ec, const size_t bytes){
				LOG_TRACE << "session[" << self->_id << "] write(#RESP_" << self->_mgr->getFuncName((*msg)[4]) << "#): "
						<< util::to_hex(*msg);

				self->_sque.pop_front();
				if (ec) {
					LOGF_ERROR << "session[" << self->_id << "] write failed(" << bytes << "B): [" << util::to_hex(*msg)
							<< "] [error: " << ec.message() << "]";
					return;
				}
...
bool CSession::checkHead()
{
	...
	LOG_ERROR << "session[" << _id << "] read header error: " << std::hex
			<< " 0x"  << (int)_hdr.ucHead1
			<< " | 0x" << (int)_hdr.ucHead2
...
void CSession::onReqLogin(const std::shared_ptr<CReqLoginPkt>& req)
{
	...
	if (!_logined) {
		...
		if (_logined) {
			...
			resp.id(_id);
			LOG_INFO << "session[" << _id << "] (" << getType() << ") <" << guid() << "> login success.";
			...
			LOGF_ERROR << "session[" << _id << "] <" << guid() << "> login failed.";
		}
	}
	else {
		resp.error(ERRCODE::REPEAT_LOGINED_ERR);
		resp.id(_id);
		LOGF_ERROR << "session[" << _id << "] <" << guid() << "> login repeat.";
	}
...
{
	LOG_INFO << "session[" << _id << "] request proxy destination[" << req->uiDstId << "]";

	auto chann = _mgr->createChannel(shared_from_this(), req->uiDstId);
	if (!chann) {
		LOGF_ERROR << "session[" << _id << "] request proxy destination[" << req->uiDstId << "] no found.";
	...
	if (!dst_ss) {
		LOGF_ERROR << "session[" << req->uiDstId << "] invalid!";
		return;
	}
	if (!dst_ss->isRuning()) {
		LOGF_ERROR << "session[" << req->uiDstId << "] has stopped";
		return;
	}
...
void CSession::onReqGetProxies(const std::shared_ptr<CReqGetProxiesPkt>& req)
{
	...
	asyncWrite(msg);
	LOG_TRACE << "session[" << _id << "] get sessions: " << util::to_hex(*msg);
}
...
bool CSession::addSrcChannel(const ChannelPtr& chann)
{
	...
	LOG_INFO << "session[" << _id << "] has src channels: " << _src_channels.size()
...
bool CSession::addDstChannel(const ChannelPtr& chann)
{
	...
	LOG_INFO << "session[" << _id << "] has dst channels: " << _dst_channels.size()
...
{
	LOG_INFO << "session[" << _id << "] close channel: " << chann->id();
